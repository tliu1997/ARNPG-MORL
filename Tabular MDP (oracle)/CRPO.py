...
# CRPO with softmax parameterization
N = 300
tol = 0.01
...
for step in steps:
    ...
    for k in range(N):
        ...
        if vgvals < b + tol:
            gradient = grad(qgvals - vgvals, prob, d_pi)
        else:
            gradient = grad(qrvals - vrvals, prob, d_pi)
...
print('Index of best step size:', np.argmin(final_gap))
print('Gap w.r.t. best step size:', min(final_gap))


# Saving the data. This can be loaded to make the figure again.
np.savetxt('CRPO_gap_s20a10g8b3.txt', gap)
np.savetxt('CRPO_violation_s20a10g8b3.txt', violation)
