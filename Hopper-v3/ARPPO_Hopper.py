...
"""Anchor-changing Regularized PPO with Extra Primal Dual
"""


class ARPPO:
    """
    Implement ARPPO algorithm
    """
    ...
    def __init__(self,
        ...
        self.anchor_policy = copy.deepcopy(policy_net)
    ...
    def update_params(self, rollout, dtype, device, update_nu = False):
        ...
        # Get log likelihood, mean, and std of anchor policy
        ar_logprob, ar_mean, ar_std = self.anchor_policy.logprob(obs, act)
        ar_logprob, ar_mean, ar_std = to_dytype_device(dtype, device, ar_logprob, ar_mean, ar_std)
        ar_logprob, ar_mean, ar_std = graph_detach(ar_logprob, ar_mean, ar_std)
        ...
        dataset = torch.utils.data.TensorDataset(obs, act, vtarg, adv, cvtarg, cadv,
                                                 ...
                                                 ar_logprob, ar_mean, ar_std)
        loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=self.mb_size, shuffle=True)
        avg_cost = rollout['avg_cost']

        # Update nu
        if update_nu:
            self.update_tmp = self.logger.hyperparams['tk'] * self.nu_lr * (avg_cost - self.cost_lim) * 0.8
            self.nu = max(self.nu + self.update_tmp, -self.update_tmp)
        
        for epoch in range(self.num_epochs):
            for _, (obs_b, act_b, vtarg_b, adv_b, cvtarg_b, cadv_b,
                    old_logprob_b, old_mean_b, old_std_b,
                    ar_logprob_b, ar_mean_b, ar_std_b) in enumerate(loader):
                ...
                kl_reg = gaussian_kl(mean, std, ar_mean_b, ar_std_b)
                ratio = torch.exp(logprob - old_logprob_b)
                self.pi_loss = ((kl_new_old - (1 / self.lam) * ratio * (adv_b - (self.nu + self.update_tmp) * cadv_b)) * (kl_new_old.detach() <= self.eta) \
                                    + self.logger.hyperparams['alpha'] * kl_reg).type(dtype)
...
def train(args):
    ...
    agent = ARPPO(env, policy, value_net, cvalue_net,
    ...
    for iter in range(args.max_iter_num):
        ...
        if iter % hyperparams['tk'] == 0:
            update_nu = True
            agent.anchor_policy.load_state_dict(policy.state_dict())
        else:
            update_nu = False
        ...
        agent.update_params(rollout, dtype, device, update_nu)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env-id', default='Hopper-v3',
                        help='Name of Environment (default: Hopper-v3')
    ...
    parser.add_argument('--alpha', type=float, default=0.05,
                        help='anchor regularization (default: 0.05)')
    parser.add_argument('--tk', type=int, default=2,
                        help='Inner loop tk (default: 2)')
