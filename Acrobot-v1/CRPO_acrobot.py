...
"""CRPO Method with Softmax Parametrization
"""
...
def step(env, policy_model, value_function_model, cost_value_function_model, cost_value_function_model2, gamma,
    ...
    for batch_num in range(num_batches):
        ...
        # (different from normal_trpo, add cost)
        ...
        if total_cost <= limit + tolerance:
            cost_surrogate_loss.backward(retain_graph=True)
        elif total_cost2 <= limit2 + tolerance:
        ...
        if policy_gradient.nonzero().size()[0]:
            ...
            if total_cost <= limit + tolerance:
                theta = linesearch(parameters_to_vector(policy_model.parameters()), policy_model, observations, actions,
    ...
    return total_reward, total_cost, total_cost2
...
def main():
    ...
    limit = -50  # I: 50
    limit2 = -50  # I: 50
    tolerance = -0.5  # accept range -10 to -0.5
    ...
    for iteration in range(STEP):
        result, violation, violation2 = step(env, policy_model, value_function_model, cost_value_function_model,
                                             cost_value_function_model2, gamma,
                                             episodes, length, batch_size, max_kl, cg_iters, residual_tol, cg_damping,
                                             ent_coeff,
                                             penality, penality2, limit, limit2, tolerance)

        if -1 * violation > -1 * limit and -1 * violation2 > -1 * limit2:
            max_kl = 0.01
        elif -1 * violation > -1 * limit or -1 * violation2 > -1 * limit2:
            max_kl = 0.006
        else:
            max_kl = 0.003

        # if result > 150: tolerance = 2   uncommen this line sometimes yields better empirical performance
    ...
    plt.plot(iterations, violations2, color='g', linestyle='-', label='C-TRPO cost2')

    # plt.plot(iterations, results_avg, color='r', linestyle='-.', label='C-TRPO average reward')
    plt.plot(iterations, violations_avg, color='b', linestyle='-.', label='C-TRPO average cost')
    ...
    plt.savefig('CRPO.png')
    plt.show()

    np.savetxt('CRPO_reward.txt', results)
    np.savetxt('CRPO_cost.txt', violations)
    np.savetxt('CRPO_cost2.txt', violations2)
