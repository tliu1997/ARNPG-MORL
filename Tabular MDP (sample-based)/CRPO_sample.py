...
"""CRPO Method with Softmax Parametrization (sample-based)
"""
...
for k in range(N):
    ...
    qrest = savalue_est(prob_transition, prob, reward, gamma, s, a, Ksamples)
    qgest = savalue_est(prob_transition, prob, utility, gamma, s, a, Ksamples)
    vrest = np.zeros(s)
    vgest = np.zeros(s)
    for j in range(s):
        vrest[j] = np.dot(qrest[j * a: (j + 1) * a], prob[j * a: (j + 1) * a])
        vgest[j] = np.dot(qgest[j * a: (j + 1) * a], prob[j * a: (j + 1) * a])
    vrvals = np.dot(np.transpose(vrest), rho)
    vgvals = np.dot(np.transpose(vgest), rho)

    npgradest = np.zeros(s * a)
    if vgvals < b + tol:
        for j in range(s * a):
            npgradest[j] = qgest[j] - vgest[np.floor_divide(j, a)]
    else:
        for j in range(s * a):
            npgradest[j] = qrest[j] - vrest[np.floor_divide(j, a)]
...
np.savetxt('CRPO_gap_s20a10g8b3k20.txt', gap)
np.savetxt('CRPO_violation_s20a10g8b3k20.txt', violation)
